...
class HashTagView: UIView {
    ...
    var _attrNSString: NSMutableAttributedString? {
        didSet {
            ...
            _attrNSString?.addAttribute(NSAttributedString.Key.font, value: textViewFont, range: range)
            //outputVC時，改變tag顏色
    ...
    fileprivate func setUpHashTagTextView() {
        ...
        hashTagTextView.linkTextAttributes = [NSAttributedString.Key.foregroundColor: tagcolor]
    ...
    fileprivate func setText(of text: String) {
        ...
        attrNSString.addAttribute(NSAttributedString.Key.font, value: textViewFont, range: range)
        attrNSString.addAttribute(NSAttributedString.Key.foregroundColor, value: textcolor, range: range)
    ...
    private func setAttribute(of word: String){
        ...
        while searchRange.location < fullStringLength {
            ...
            if  previousCharacter == "" || previousCharacter == "\0" || previousCharacter == " " || previousCharacter == "\n" {
                ...
                attrNSString.addAttribute(NSAttributedString.Key.link, value: "\(encodedInputString)", range: foundRange)
                //在inputVC時就改變tag顏色
